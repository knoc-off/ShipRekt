...
public class FlackScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (inCollider)
        {
            //print("incol " + Time.fixedTime);
            //tempbool = true;
            if (Input.GetButton("Fire1") && shot && Time.fixedTime > frames && ammo > 0)
            {
                //print("click " + Time.fixedTime);
                shot = false;
            ...
            if ((RotateTo(angle) && (Time.fixedTime > frames && ammo > 0)) && !shot)
            {
                //print("rot " + Time.fixedTime);
    ...
    bool RotateTo(float angle)
    {
        ...
        if (Mathf.Abs(Mathf.DeltaAngle(transform.localEulerAngles.z, angle)) < 20)
            return true;
        return false;
    }
    void shoot()
    {
        ...
        GameObject bullet = Instantiate(bulletPrefab, abc.position, abc.rotation);
        Rigidbody2D rb = bullet.GetComponent<Rigidbody2D>();
        rb.gameObject.GetComponent<Transform>().eulerAngles += new Vector3(0, 0, 90);
        rb.AddForce(abc.up * force, ForceMode2D.Impulse);
    }
